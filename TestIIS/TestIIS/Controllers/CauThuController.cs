    ...
    public class CauThuController : Controller
    {
        ...
        public ActionResult Create(string maCT, string hoTen, string maDB)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                string checkSql = "";
                if (isSV1) {
                   checkSql = "SELECT COUNT(*) FROM CauThu WHERE MaCT = @maCT";
                }else
                {
                   checkSql = @"SELECT 1 FROM (SELECT MaCT FROM CauThu WHERE MaCT = @maCT
                                UNION
                                SELECT MaCT FROM LINK.BongDa.dbo.CauThu WHERE MaCT = @maCT) AS T";
                }
